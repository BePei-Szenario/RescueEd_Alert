...
export async function currentRegistrationDocuments(){
 ...
 for(const documentKey of REQUIRED_REGISTRATION_DOCUMENTS){
  ...
  if(!snapshot){
   await db.insert(legalDocumentVersions).values({id:id("ldv"),documentKey,title:current.title,version:current.version,content:current.content,contentHash:hash,publishedAt:current.updatedAt,createdAt:current.updatedAt}).onConflictDoNothing();
  ...
  if(!snapshot||snapshot.contentHash!==hash||snapshot.content!==current.content||snapshot.title!==current.title||snapshot.archivedAt)return null;
