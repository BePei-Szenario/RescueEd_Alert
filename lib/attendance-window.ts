type TimedEvent={status:string;eventDate:string;endDate?:string|null;startTime:string|null;endTime:string|null};
...
export function attendanceWindowOpen(event:TimedEvent,now=new Date()){
 ...
 const overnight=end<=start,endDate=event.endDate||(overnight?addDays(event.eventDate,1):event.eventDate);
