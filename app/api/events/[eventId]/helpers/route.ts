...
export async function POST(request:Request,{params}:{params:Promise<{eventId:string}>}){
 try{
  ...
  const inserted=await db.run(sql`INSERT INTO helpers (id,event_id,assignment_id,name,first_name,last_name,qualification,session_token_hash,registered_at,removed_at)
    SELECT ${helperId},${event.id},NULL,${name},${firstName},${lastName},${qualification},${sessionTokenHash},${registeredAt.getTime()},NULL
  ...
  if((inserted.meta?.changes??0)!==1)return Response.json({error:"Die maximale Helferzahl ist erreicht oder diese Person bereits anwesend."},{status:409});
