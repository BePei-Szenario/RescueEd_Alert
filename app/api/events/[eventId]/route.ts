...
import {assignments,emailOutbox,events,helpers,invoiceRequests} from "@/db/schema";
...
export async function GET(request:Request,{params}:{params:Promise<{eventId:string}>}){
 try{
  ...
  const units=await db.select().from(assignments).where(eq(assignments.eventId,event.id)).orderBy(asc(assignments.name));
  const people=await db.select({id:helpers.id,name:helpers.name,firstName:helpers.firstName,lastName:helpers.lastName,qualification:helpers.qualification,assignmentId:helpers.assignmentId,registeredAt:helpers.registeredAt,removedAt:helpers.removedAt}).from(helpers).where(eq(helpers.eventId,event.id));
...
export async function DELETE(request:Request,{params}:{params:Promise<{eventId:string}>}){
 ...
 try{
  ...
  const pdf=createEventSummaryPdf({event,invoice:invoice||null,helpers:people,generatedAt:new Date()});
  const recipientEmail=invoice?.email||user.email,filename=`rescueed-eventabschluss-${event.id}.pdf`,now=new Date();
