...
import {alertAssignments,alertRecipients,alerts,assignments,emailOutbox,eventAccessCodes,events,helpers,invoiceRequests,organizations,users} from "@/db/schema";
...
export async function GET(request:Request,{params}:{params:Promise<{eventId:string}>}){
 try{
  ...
  const alarmStatusRows=await db.select({assignmentId:alertAssignments.assignmentId,helperId:alertRecipients.helperId,alertCreatedAt:alerts.createdAt,acknowledgedAt:alertRecipients.acknowledgedAt}).from(alertRecipients).innerJoin(alerts,eq(alerts.id,alertRecipients.alertId)).innerJoin(helpers,eq(helpers.id,alertRecipients.helperId)).innerJoin(alertAssignments,and(eq(alertAssignments.alertId,alerts.id),eq(alertAssignments.assignmentId,helpers.assignmentId))).where(eq(alerts.eventId,event.id));
  const latestAlarmByAssignment=new Map<string,number>();
  for(const row of alarmStatusRows){const createdAt=row.alertCreatedAt.getTime(),latest=latestAlarmByAssignment.get(row.assignmentId);if(latest===undefined||createdAt>latest)latestAlarmByAssignment.set(row.assignmentId,createdAt)}
  const currentAlarmStatus=new Map<string,Date|null>();
  for(const row of alarmStatusRows)if(latestAlarmByAssignment.get(row.assignmentId)===row.alertCreatedAt.getTime())currentAlarmStatus.set(`${row.assignmentId}:${row.helperId}`,row.acknowledgedAt);
  const alarmCountsByAssignment=new Map<string,{recipients:number;pending:number}>();
  for(const row of alarmStatusRows)if(latestAlarmByAssignment.get(row.assignmentId)===row.alertCreatedAt.getTime()){const counts=alarmCountsByAssignment.get(row.assignmentId)||{recipients:0,pending:0};counts.recipients+=1;if(!row.acknowledgedAt)counts.pending+=1;alarmCountsByAssignment.set(row.assignmentId,counts)}
  const visibleUnits=units.map(unit=>{const active=operationalPeople.filter(person=>!person.removedAt&&person.assignmentId===unit.id),alarmCounts=alarmCountsByAssignment.get(unit.id);return {...unit,activeHelperCount:active.length,activeQrHelperCount:active.filter(person=>person.registrationSource==="qr").length,alarmRecipientCount:alarmCounts?.recipients??0,pendingAcknowledgementCount:alarmCounts?.pending??0}});
  const visiblePeople=people.map(person=>{if(!person.assignmentId)return person;const key=`${person.assignmentId}:${person.id}`;return currentAlarmStatus.has(key)?{...person,alarmAcknowledgedAt:currentAlarmStatus.get(key)}:person});
