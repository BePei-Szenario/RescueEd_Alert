...
export async function POST(request:Request,{params}:{params:Promise<{userId:string}>}){
 ...
 try{
  ...
  const db=getDb(),[target]=await db.select({id:users.id,role:users.role,status:users.status,organizationId:users.organizationId,complimentaryAccess:organizations.complimentaryAccess}).from(users).innerJoin(organizations,eq(organizations.id,users.organizationId)).where(eq(users.id,userId)).limit(1);
  if(!target||target.role!=="customer"||target.status==="deleted")return Response.json({error:"Kundenkonto nicht gefunden."},{status:404});
