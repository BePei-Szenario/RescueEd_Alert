...
export async function POST(request:Request,{params}:{params:Promise<{userId:string}>}){
 try{
  ...
  const {userId}=await params,db=getDb(),[user]=await db.select({email:users.email}).from(users).where(eq(users.id,userId)).limit(1);
  if(!user)return Response.json({error:"Konto nicht gefunden."},{status:404});
  const raw=crypto.randomUUID()+crypto.randomUUID(),now=new Date(),expiresAt=new Date(now.getTime()+30*60*1000),senderEmail=await senderFor("password_reset");
  await db.batch([
   ...
   db.insert(emailOutbox).values({id:id("mail"),userId,type:"password_reset",senderEmail,recipientEmail:user.email,subject:"RescueEd Alert – Passwort festlegen",payloadJson:emailPayload({template:"password_link",token:raw,expiresAt:expiresAt.toISOString(),message:"Über diesen einmaligen Link können Sie Ihr Passwort sicher festlegen."}),createdAt:now}),
