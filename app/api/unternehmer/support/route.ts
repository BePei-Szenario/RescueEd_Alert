...
export async function GET(){
 try{
  ...
  const [tickets,crashes]=await Promise.all([
   db.select({id:supportTickets.id,subject:supportTickets.subject,status:supportTickets.status,requesterType:supportTickets.requesterType,requesterName:users.fullName,requesterEmail:users.email,eventId:supportTickets.eventId,createdAt:supportTickets.createdAt,updatedAt:supportTickets.updatedAt}).from(supportTickets).leftJoin(users,eq(supportTickets.requesterUserId,users.id)).where(ne(supportTickets.status,"resolved")).orderBy(desc(supportTickets.updatedAt)).limit(100),
