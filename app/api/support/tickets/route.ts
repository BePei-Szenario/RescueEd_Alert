...
export async function POST(request:Request){
 try{
  ...
  const body=JSON.parse(raw) as {eventId?:unknown;subject?:unknown;message?:unknown;appVersion?:unknown;platform?:unknown;buildMode?:unknown};
  ...
  const hasTechnicalData=body.appVersion!==undefined||body.platform!==undefined||body.buildMode!==undefined;
  const appVersion=typeof body.appVersion==="string"?body.appVersion.trim():"";
  const platform=typeof body.platform==="string"?body.platform.trim():"";
  const buildMode=typeof body.buildMode==="string"?body.buildMode.trim():"";
  if(hasTechnicalData&&(!/^[-\w.+() ]{1,40}$/.test(appVersion)||!["android","ios","other"].includes(platform)||!["release","profile","debug"].includes(buildMode)))return Response.json({error:"Technische App-Angaben sind ungültig."},{status:400});
  const storedMessage=hasTechnicalData?`${message}\n\n---\nTechnische Angaben\nApp-Version: ${appVersion}\nPlattform: ${platform}\nAusführungsmodus: ${buildMode}`:message;
  ...
  await db.batch([
   ...
   db.insert(supportMessages).values({id:id("msg"),ticketId,authorType:"requester",body:storedMessage,createdAt:now})
  ]);
