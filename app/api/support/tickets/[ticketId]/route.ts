...
async function access(request:Request,ticketId:string,eventId?:string|null){
 ...
 const owner=await platformOwner();if(owner)return {ticket,owner:true};
...
export async function POST(request:Request,{params}:{params:Promise<{ticketId:string}>}){
 try{
  ...
  const db=getDb(),now=new Date();await db.batch([db.insert(supportMessages).values({id:id("msg"),ticketId,authorType:owner?"support":"requester",body:message,createdAt:now}),db.update(supportTickets).set({updatedAt:now,status:owner?"in_progress":ticket.status}).where(eq(supportTickets.id,ticketId))]);
...
export async function PATCH(request:Request,{params}:{params:Promise<{ticketId:string}>}){
 try{
  const bad=rejectCrossSiteMutation(request);if(bad)return bad;
  const owner=await platformOwner();if(!owner)return Response.json({error:"Nicht autorisiert."},{status:401});
  ...
  const updated=await getDb().update(supportTickets).set({status:body.status as "open"|"in_progress"|"resolved",updatedAt:new Date()}).where(eq(supportTickets.id,ticketId)).returning({id:supportTickets.id});
