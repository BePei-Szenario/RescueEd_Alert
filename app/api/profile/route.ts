...
export async function GET(){
 ...
 try{
  const db=getDb(),[organizationRows,acknowledgements,members]=await Promise.all([
  ...
  return Response.json({profile:{id:account.id,name:account.fullName,email:account.email,role:account.role,organization:organization?.name??null,organizationType:organization?.organizationType??null},acknowledgements:acknowledgements.map(row=>({...row,acceptedAt:row.acceptedAt.toISOString(),own:row.signerId===account.id})),members:members.map(member=>({...member,emailVerifiedAt:member.emailVerifiedAt?.toISOString()??null}))},{headers:{"cache-control":"no-store"}});
