...
export async function POST(request:Request){
 try{
  ...
  const body=await request.json() as {organization?:string;organizationType?:unknown;contactName?:string;street?:string;houseNumber?:string;postalCode?:string;city?:string;email?:string;acceptedDocumentVersionIds?:string[]};
  ...
  if(!isOrganizationType(body.organizationType))return Response.json({error:"Bitte eine gültige Organisationsart auswählen."},{status:400});
