...
export async function POST(request:Request){
 try{
  ...
  return new Response(JSON.stringify({ok:true,userId:user.id,role:user.role,redirectTo:user.role==="platform_owner"?"/unternehmer":"/"}),{headers:{"content-type":"application/json","cache-control":"no-store","set-cookie":`rescueed_session=${raw}; HttpOnly; SameSite=Strict; Path=/; Max-Age=28800${secure}`}});
