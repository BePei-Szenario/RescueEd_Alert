...
type Document={id:string;documentKey:string;title:string;version:string;content:string;pdfFileName?:string|null;pdfUrl?:string|null;acknowledgementType:"read"|"accepted"};
...
export default function LegalUpdatePage(){
 ...
 const [review,setReview]=useState<Review|null>(null),[checked,setChecked]=useState<string[]>([]),[error,setError]=useState(""),[busy,setBusy]=useState(false);
 ...
 return <main className="legal-update-page"><header><RescueEdLogo/><b>RescueEd Alert</b><Link href="/">← Zu meinen Events</Link></header><section className="legal-update-card"><p className="legal-update-kicker">RECHTLICHE FASSUNGEN</p><h1>Aktuelle Rechtstexte</h1><p>Laufende Events bleiben erreichbar. Vor einem neuen Event muss das Hauptkonto die aktuellen Fassungen bestätigen.</p>{!review&&!error&&<p>Fassungen werden geladen …</p>}{error&&<p className="legal-update-error" role="alert">{error}</p>}{review&&!review.available&&<p>Die Rechtstexte sind noch nicht vollständig veröffentlicht. Bitte später erneut prüfen.</p>}{review?.available&&!review.required&&<p>Alle aktuellen Fassungen sind bereits bestätigt.</p>}{review?.required&&!review.canConfirm&&<p>Bitte das Hauptkonto Ihrer Organisation bitten, die aktuellen Fassungen zu bestätigen.</p>}{review?.required&&review.canConfirm&&<><p>Öffnen und lesen Sie die Texte. Setzen Sie anschließend bei jeder Fassung das Häkchen.</p><div className="legal-update-documents">{review.documents.map(document=><article key={document.id}><details><summary>{document.title} · Version {document.version}</summary>{document.pdfUrl&&<><a className="legal-update-pdf-link" href={document.pdfUrl} target="_blank" rel="noreferrer">PDF in neuem Fenster öffnen</a><iframe className="legal-update-pdf" src={document.pdfUrl} title={`${document.title}, Version ${document.version}`}/></>}<div className="legal-update-content">{document.content}</div></details><label><input type="checkbox" disabled={busy} checked={checked.includes(document.id)} onChange={event=>setChecked(items=>event.target.checked?[...items,document.id]:items.filter(item=>item!==document.id))}/><span>{document.acknowledgementType==="read"?`Ich habe ${document.title} gelesen.`:`Ich habe ${document.title} gelesen und akzeptiere diese Fassung.`}</span></label></article>)}</div><button disabled={busy||checked.length!==review.documents.length} onClick={confirm}>{busy?"Wird gespeichert …":"Alle Fassungen bestätigen"}</button></>}<Link className="legal-update-back" href="/">Laufendes Event fortsetzen</Link></section></main>;
}
