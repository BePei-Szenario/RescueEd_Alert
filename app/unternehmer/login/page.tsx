...
export default function UnternehmerLogin(){
 const [step,setStep]=useState<"login"|"mfa">("login"),[email,setEmail]=useState(""),[password,setPassword]=useState(""),[code,setCode]=useState(""),[userId,setUserId]=useState(""),[message,setMessage]=useState(""),[busy,setBusy]=useState(false);
 async function submit(e:FormEvent){e.preventDefault();setBusy(true);setMessage("");try{const url=step==="login"?"/api/auth/login":"/api/auth/mfa/verify",body=step==="login"?{email,password,area:"unternehmer"}:{userId,code,area:"unternehmer"};const res=await fetch(url,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(body)}),data=await res.json();if(!res.ok)throw new Error(data.error||"Anmeldung fehlgeschlagen.");if(step==="login"){setUserId(data.userId);setStep("mfa");if(data.previewCode)setMessage(`Lokaler Sicherheitscode: ${data.previewCode}`)}else location.href="/unternehmer"}catch(err){setMessage(err instanceof Error?err.message:"Anmeldung fehlgeschlagen.")}finally{setBusy(false)}}
