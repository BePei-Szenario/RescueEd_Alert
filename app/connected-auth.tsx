...
export function ConnectedLogin({back,register}:{back:()=>void;register:()=>void}){
 ...
 return <Shell back={back} title={phase==="login"?"Anmelden":"Sicherheitscode"} copy={phase==="login"?"Melde dich mit deinem RescueEd Alert Konto an. Dein Bereich öffnet sich automatisch.":"Gib den sechsstelligen Sicherheitscode ein."}>{phase==="login"?<form key="login" className="authform" onSubmit={login}><Field label="E-Mail-Adresse" icon={<Mail/>}><input name="organizationEmail" required type="email" autoComplete="username" value={email} onChange={e=>setEmail(e.target.value)}/></Field><Field label="Passwort" icon={<LockKeyhole/>}><PasswordField name="organizationPassword" required autoComplete="current-password" value={password} onChange={e=>setPassword(e.target.value)}/></Field>{error&&<p className="autherror" role="alert">{error}</p>}<button>Anmelden <ArrowRight size={18}/></button><button type="button" className="textlink" onClick={switchAccount}>Anderes Konto verwenden</button></form>:<form key="security-code" className="authform" autoComplete="off" onSubmit={verify}><Field label="Sicherheitscode" icon={<KeyRound/>}><input name="code" required inputMode="numeric" autoComplete="one-time-code" maxLength={6}/></Field><p className="authhint">{hint}</p>{error&&<p className="autherror" role="alert">{error}</p>}<button>Sicherheitscode bestätigen <ArrowRight size={18}/></button></form>}{phase==="login"&&<Link className="textlink" href="/passwort-vergessen">Passwort vergessen?</Link>}<button className="textlink" onClick={register}>Noch kein Konto? Jetzt registrieren</button></Shell>;
}
